...
class Boss(Enemy):
    def __init__(self, realm, base_data):
        super().__init__(realm, [base_data])
        self.name = f"{base_data['name']} (BOSS)"
        self.max_hp = int(base_data['hp'] * (2 + realm))
        self.hp = self.max_hp
        self.damage = int(base_data['damage'] * (1.5 + realm * 0.5))
        self.exp = int(base_data['exp'] * 5)
        self.gold = int(base_data['gold'] * 5)
        self.size = base_data['size'] + 20
